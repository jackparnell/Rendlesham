class TowerInfo extends GameState
{
    create()
    {
        ...
        for (let i = 0; i < towers.length; i++)
        {
            ...
            $('#' + tableId).append('<tr><th>Grade</th><th>Damage Value</th><th>Fire Rate (ms)</th><th>Range</th></tr>');

            for (let j = 1; j <= 3; j++)
            {
                $('#' + tableId).append('<tr><td>' + towers[i].grade + '</td><td>' + towers[i].bulletDamageValue + '</td><td>' + Math.round(towers[i].weapon1.fireRate) + '</td><td>' + towers[i].getRangeInTiles().toFixed(1) + '</td></tr>');
                towers[i].upgrade();
            }
